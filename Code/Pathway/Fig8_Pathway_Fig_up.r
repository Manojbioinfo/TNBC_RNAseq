...
upregulated_pathways <- final_pathway_data %>%
  filter(grepl("Up", Dataset, fixed = TRUE))

downregulated_pathways <- final_pathway_data %>%
  filter(grepl("Down", Dataset, fixed = TRUE))

# View the results
cat("=== UPREGULATED PATHWAYS (TCGA_Up) ===\n")
print(upregulated_pathways)
...
final_pathway_data=upregulated_pathways 
...
p
# Save plots
ggsave("Fig2.Top10_Pathways_GO_up.pdf", plot = p, device = "pdf", width = 5, height = 7)

# Save JPEG version (300 DPI)
ggsave("Fig2.Top10_Pathways_GO_up.jpg", plot = p,

...
sample_info_pre$gender=sample_info_pre$gender.ch1
sample_info_pre$age=sample_info_pre$age_numeric
pca_df <- as.data.frame(pca$x[,1:5])
pca_df$sample_id <- rownames(pca_df)
pca_df <- left_join(pca_df,
                    sample_info_pre %>% dplyr::select(sample_id, condition, gender, age),
                    by = "sample_id")
...
saveRDS(p_pca, "GSE38959.PCA.rds")
...
saveRDS(flagged_samples, "GSE38959.flagged_samples.rds")
...
saveRDS(p_pca, "GSE38959.PCA.age.rds")
